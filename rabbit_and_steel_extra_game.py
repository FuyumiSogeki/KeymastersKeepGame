...
@dataclass
class RabbitAndSteelExtraArchipelagoOptions:
    rabbit_and_steel_extra_include_extra: RabbitAndSteelExtraIncludeExtra
    rabbit_and_steel_extra_unlocked_characters: RabbitAndSteelExtraUnlockedCharacters
    rabbit_and_steel_extra_allow_lunar: RabbitAndSteelExtraAllowLunar


class RabbitAndSteelExtraGame(Game):
    name = "Rabbit & Steel (Extra)"
    platform = KeymastersKeepGamePlatforms.PC

    is_adult_only_or_unrated = False

    options_cls = RabbitAndSteelExtraArchipelagoOptions
    ...
    def game_objective_templates(self) -> List[GameObjectiveTemplate]:
        ...
        if self.archipelago_options.rabbit_and_steel_extra_include_extra.value:
            templates.extend([
                GameObjectiveTemplate(
                    label="Scale the Reflecting Pool and defeat The Witch of Sorrowed Heart on DIFF difficulty",
                    data={"DIFF": (self.difficulties, 1)},
                    is_time_consuming=True,
                    is_difficult=False, # Depends on optional factors, very easy on Easy
                    weight=2
                ...
                GameObjectiveTemplate(
                    label="Scale the Reflecting Pool and defeat The Witch of Sorrowed Heart as the CHARACTER",
                    data={"CHARACTER": (self.characters, 1)},
                    is_time_consuming=True,
                    is_difficult=False,
                    weight=2
                ...
                GameObjectiveTemplate(
                    label="Scale the Reflecting Pool and defeat The Witch of Sorrowed Heart on DIFF difficulty as the CHARACTER",
                    data={"DIFF": (self.difficulties, 1), "CHARACTER": (self.characters, 1)},
                    is_time_consuming=True,
                    is_difficult=False,
                    weight=2
                ...
                GameObjectiveTemplate(
                    label="Pet Tassha, the Snowfur Wolf",
                    data={},
                    is_time_consuming=True,
                    is_difficult=False,
                    weight=1
                ...
                GameObjectiveTemplate(
                    label="Try to pet Saya, the Archon Crow",
                    data={},
                    is_time_consuming=True,
                    is_difficult=False,
                    weight=1
    ...
    def characters(self):
        ...
        characters.extend(self.archipelago_options.rabbit_and_steel_extra_unlocked_characters.value)
        
        if not self.archipelago_options.rabbit_and_steel_extra_include_extra:
            characters.extend(self.archipelago_options.rabbit_and_steel_extra_unlocked_characters.value)
            for extra_character in self.characters_extra:
                while extra_character in characters:
                    characters.remove(extra_character)
    ...
    def difficulties(self):
        ...
        if self.archipelago_options.rabbit_and_steel_extra_allow_lunar:
    ...
    def stages(self):
        ...
        if self.archipelago_options.rabbit_and_steel_extra_include_extra:
            stages.extend(self.extra_stages)
            
        return stages


class RabbitAndSteelExtraUnlockedCharacters(OptionSet):
...
class RabbitAndSteelExtraAllowLunar(Toggle):
...
class RabbitAndSteelExtraIncludeExtra(DefaultOnToggle):
    """Whether the extra stages and characters should be options for objectives in Rabbit & Steel"""
    display_name = "Rabbit & Steel Include Extra Mode"
